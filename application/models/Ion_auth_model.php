...
class Ion_auth_model extends CI_Model
{
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, level, DATE_FORMAT(FROM_UNIXTIME(created_on),"%d-%m-%Y %H:%i:%s") as created_on, first_name, last_name, photo')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
    ...
    public function get_menu($identity)
    {
        ...
        $this->db->select('a.menuid, m.name as menuname, m.img as menuimg, a.id as appid, a.name as appname, a.function as appfunction, a.leaf as menuleaf, a.visible')
    ...
    public function asocia_comunidad($userid, $array_comunidades, $elimina_anterior = true)
    {
      ...
      //  var_dump($array_comunidades); exit;
        ...
        } else if (count($array_comunidades) == 1) {
            $this->db->query('insert ignore into gc_usuario_comunidad (idusuario,idcomunidad) values (' . $userid . ',' . $array_comunidades[0] . ')');
        }
    }
